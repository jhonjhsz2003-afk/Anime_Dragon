...
const avatar = a => /^\/assets\/avatars\/avatar-([1-9]|1[0-2])\.svg$/.test(a || '') ? a : '/assets/avatars/avatar-1.svg';
const publicUser = u => ({id:u.id,name:u.username,email:u.email,bio:u.bio || '',avatar:avatar(u.avatar_url)});
...
async function getUser(r,env) {
  const token=tokenFrom(r);if(!token)return null;
  return env.DB.prepare('SELECT users.* FROM sessions JOIN users ON users.id=sessions.user_id WHERE sessions.id=? AND sessions.expires_at>?').bind(await digest(token),new Date().toISOString()).first();
}
...
async function createSession(r,env,u) {
  ...
  const statements=[env.DB.prepare('INSERT INTO sessions(id,user_id,expires_at,created_at) VALUES(?,?,?,?)').bind(await digest(token),u.id,expires.toISOString(),now.toISOString())];
  if(old)statements.push(env.DB.prepare('DELETE FROM sessions WHERE id=?').bind(await digest(old)));
  await env.DB.batch(statements);
  return reply({ok:true,user:publicUser(u)},200,{'Set-Cookie':cookie(r,token,604800)});
}
export async function auth(request,env) {
  if(!env.DB || !env.AUTH_SECRET || env.AUTH_SECRET.length<32) throw error(503,'O acesso à conta está temporariamente indisponível.');
  ...
  if(path==='/api/auth/logout') {
    const t=tokenFrom(request);if(t)await env.DB.prepare('DELETE FROM sessions WHERE id=?').bind(await digest(t)).run();
  ...
  if(path==='/api/auth/profile') {
    ...
    try {await env.DB.prepare('UPDATE users SET username=?,bio=?,avatar_url=?,updated_at=? WHERE id=?').bind(name,bio,avatar(data.avatar),new Date().toISOString(),u.id).run();}
    catch(e){if(String(e.message).includes('UNIQUE'))throw error(409,'Esse nome já está em uso.');throw e;}
    return reply({ok:true,user:publicUser({...u,username:name,bio,avatar_url:avatar(data.avatar)})});
  }
  ...
  if(path==='/api/auth/login') {
    ...
    if(!u||!equal(hash,u.password_hash))throw error(401,'E-mail ou senha incorretos.');
  ...
  try {await env.DB.prepare('INSERT INTO users(id,username,email,password_hash,password_salt,avatar_url,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)').bind(u.id,name,email,hash,salt,u.avatar_url,now,now).run();}
