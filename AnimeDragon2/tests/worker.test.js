...
test('unconfigured source has a truthful unavailable response',async()=>{const r=await call('/api/playback?id=1&season=1&episode=1');assert.equal(r.status,200);assert.equal((await r.json()).available,false);assert.equal(r.headers.get('Cache-Control'),'no-store');});
