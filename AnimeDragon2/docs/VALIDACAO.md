...
Atualização v9.3: 37 testes passaram, incluindo reprodução prévia do erro 503 com o CREATE TABLE da instalação do usuário. Após a correção: cadastro com ID numérico, sessão em token_hash, consulta de sessão, logout, login, nome duplicado, preservação de registros e chaves estrangeiras, respeito ao bloqueio, propriedade de perfil/comentário com IDs numéricos, upload de foto e reinicialização idempotente. A primeira criação, incluindo migração, ficou dentro de 50 consultas no teste. Inspeção visual do modal confirmou um único botão Voltar em uma área de aproximadamente 83px, sem a faixa escura larga. A nova versão ainda precisa ser publicada para confirmação no D1 remoto.
