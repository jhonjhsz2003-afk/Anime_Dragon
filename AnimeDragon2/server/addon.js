...
async function read(url,ttl=600000,fresh=false) {
 return metadata.get(url,ttl,async()=>{
 const r=await fetch(url,{headers:{Accept:'application/json'},signal:AbortSignal.timeout(url.includes('/stream/')?10000:2500)});
...
async function resolvePlayback(anime,season,episode,env,origin,fresh) {
 ...
 const streams=(data.streams||[]).filter(s=>publicHttps(s.url)&&(!s.behaviorHints?.proxyHeaders||canRelaySource(s))).slice(0,12).map((s,i)=>({
 ...
 return {ok:true,available:streams.length>0,streams,...(streams[0]||{}),reason:streams.length?'':data.streams?.some(s=>s.behaviorHints?.proxyHeaders)?'Esta fonte exige um aplicativo ou servidor de vídeo compatível; não oferece reprodução direta neste navegador.':'Nenhuma fonte HTTPS direta foi encontrada para este episódio.'};
}
