...
export function normalizeSubtitles(rows){
 ...
 for(const t of Array.isArray(rows)?rows:[]){
   ...
   const language=languages[t.lang]||(/^[a-z]{2,3}(?:-[a-zA-Z]{2})?$/.test(t.lang||'')?t.lang:'und');
...
export async function addonSubtitles(anime,season,episode,env){
 ...
 try{
   ...
   const m=await manifest({STREMIO_MANIFEST_URL:p.url}),videoId=`${id}:${target.season}:${target.episode}`;
