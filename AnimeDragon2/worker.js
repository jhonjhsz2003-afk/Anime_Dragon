...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === '/api/health') return json({ok:true,service:'AnimeDragon',version:'9.6.0'});
      ...
      if (url.pathname === '/api/subtitles') {
        ...
        return json(await addonSubtitles(await animeDetail(id,env),Number(s),Number(ep),env));
      }
      ...
      const cacheUrl=new URL(url);cacheUrl.searchParams.set('_catalogVersion','9.6');
