...
async function catalog(request, env) {
  ...
  if (p === '/api/catalog/home') {
    ...
    const [current,recent,airing]=await Promise.all([
      discover(env,{sort_by:'popularity.desc',...dates}),
      discover(env,{sort_by:'first_air_date.desc','first_air_date.gte':from.toISOString().slice(0,10),'first_air_date.lte':today.toISOString().slice(0,10)}),
      discover(env,{sort_by:'popularity.desc','air_date.gte':week.toISOString().slice(0,10),'air_date.lte':today.toISOString().slice(0,10)}).catch(()=>({results:[]}))
    ]);
    ...
    const updated=details.filter(p=>p?.last_episode_to_air?.air_date>=week.toISOString().slice(0,10)&&p.last_episode_to_air.air_date<=today.toISOString().slice(0,10)).sort((a,b)=>b.last_episode_to_air.air_date.localeCompare(a.last_episode_to_air.air_date));
    const featured=[...updated,...recent.results,...trending].filter((p,i,all)=>all.findIndex(x=>x.id===p.id)===i).slice(0,6);
    return {ok:true,trending,anime:trending,featured,updated,top:[...trending].sort((a,b)=>b.vote_average-a.vote_average),recent:recent.results};
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === '/api/health') return json({ok:true,service:'AnimeDragon',version:'9.5.0'});
      if (url.pathname === '/api/playback/providers') return json({ok:true,providers:providers(env).map(({id,name})=>({id,name}))});
      ...
      const cacheUrl=new URL(url);cacheUrl.searchParams.set('_catalogVersion','9.5');
