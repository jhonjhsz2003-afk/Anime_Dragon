...
import {createPlaybackWatchdog,preferredAlternative} from './playback-watchdog.js?v=9.6.1';
...
export function mountWatchPlayer(host, options) {
  ...
  const watchdog=createPlaybackWatchdog(()=>{if(!dead)failure('Nenhuma versão conseguiu iniciar a reprodução. Tente novamente em instantes.');});
  ...
  function failure(message) {if(dead)return;sourceFailed=true;watchdog.stop();const alternative=sources.findIndex((s,i)=>i!==selected&&!attempted.has(sourceId(s)));if(alternative>=0){status.textContent='Abrindo outra versão do vídeo…';void selectSource(alternative);return;}if(!providersComplete){note('Preparando episódio','Estamos tentando abrir o vídeo. Aguarde um momento…');status.textContent='Preparando episódio…';return;}note('Não foi possível iniciar','As opções disponíveis não conseguiram reproduzir este episódio. Tente novamente em instantes.',true);status.textContent='';screen.classList.remove('is-playing');}
  ...
  async function begin(version){
    try{const result=await startMedia(video,()=>!dead&&version===generation);if(dead||version!==generation)return;
      ...
      else if(result==='blocked'){watchdog.stop();q('.watch-notice').hidden=true;q('.watch-big-play').hidden=false;status.textContent='Seu navegador pede um toque em reproduzir para iniciar.';}
  ...
  on(video,'canplay',()=>{watchdog.progress();q('.watch-notice').hidden=true;q('.watch-big-play').hidden=!video.paused;});
  on(video,'playing',()=>{started=true;sourceFailed=false;screen.classList.add('is-playing');q('.watch-notice').hidden=true;q('.watch-big-play').hidden=true;q('[data-play]').textContent='Ⅱ';q('[data-play]').setAttribute('aria-label','Pausar');watchdog.stop();status.textContent=video.muted?'Reproduzindo sem som. Use Ativar som ou o controle de volume.':'';q('[data-enable-sound]').hidden=!video.muted;if(!activity){activity=true;Promise.resolve(o.onActivity()).catch(()=>{activity=false;});}});
  on(video,'pause',()=>{if(started)watchdog.stop();save();screen.classList.remove('is-playing');q('[data-play]').textContent='▶';q('[data-play]').setAttribute('aria-label','Reproduzir');q('.watch-big-play').hidden=!q('.watch-notice').hidden;});
  on(video,'waiting',()=>{status.textContent='Carregando vídeo…';if(started&&!video.paused)watchdog.start();});
  ...
  function destroy(){if(dead)return;dead=true;++generation;++loadGeneration;observer.disconnect();watchdog.stop();captionAbort?.abort();abort.abort();save();disposeMedia(video,hls);hls=null;captionUrls.forEach(URL.revokeObjectURL);captionUrls=[];}
