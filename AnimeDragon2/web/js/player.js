...
export function mountWatchPlayer(host, options) {
  ...
  let loadGeneration=0,prefetchedAt=0,sourceTimer=null,sourceFailed=false;const attempted=new Set();
  ...
  <div class="watch-layout"><main class="watch-main"><div class="watch-screen" tabindex="0" aria-label="Player de vídeo; espaço para reproduzir, setas para avançar ou retroceder">
    ...
    <details class="watch-help"><summary>Fontes e disponibilidade</summary><p data-provider-health>Consultando provedores de anime…</p></details>
  ...
  function failure(message) {sourceFailed=true;clearTimeout(sourceTimer);const alternative=sources.findIndex((s,i)=>i!==selected&&!attempted.has(s.url));if(alternative>=0){status.textContent='Tentando outra fonte disponível…';void selectSource(alternative);return;}note('Esta fonte não reproduziu',message,true);status.textContent=message;screen.classList.remove('is-playing');}
  ...
  async function selectSource(index, keepPosition=true){
    ...
    sourceFailed=false;attempted.add(source.url);sourceTimer=setTimeout(()=>{if(!dead&&version===generation&&video.readyState===0)failure('Esta fonte está demorando para responder. Tente atualizar ou escolher outra fonte.');},12000);
  ...
  async function load(fresh=false){
    const requestVersion=++loadGeneration;attempted.clear();
    ...
    function update(result){
      ...
      q('[data-provider-health]').textContent=(result.providers||[]).map(p=>`${p.name}: ${p.available?'fontes encontradas':p.reason||'consultando…'}`).join(' · ');
      const current=sources[selected]?.url,hadSources=sources.length>0;
      sources=(result.streams?.length?result.streams:result.url?[result]:[]).filter(s=>s.url);
      if(!sources.length){if(result.complete!==false){q('[data-source]').innerHTML='<option>Nenhuma fonte disponível</option>';note('Episódio indisponível',result.reason||'Nenhuma fonte respondeu para este episódio.',true);status.textContent=(result.providers||[]).map(p=>`${p.name}: ${p.reason}`).join(' · ');}return;}
      selected=Math.max(0,sources.findIndex(s=>s.url===current));q('[data-source]').innerHTML=sources.map((s,i)=>`<option value="${i}">${e(s.name||`Fonte ${i+1}`)}</option>`).join('');q('[data-source]').disabled=false;q('[data-source]').value=String(selected);
      if(!hadSources)void selectSource(selected,false);
      else if(sourceFailed&&sources.some(s=>!attempted.has(s.url)))failure(playbackError(video.error?.code));
    }
    try{const result=await o.loadSource({fresh,onUpdate:update});update(result);
    }catch(err){if(!dead&&requestVersion===loadGeneration){q('[data-source]').innerHTML='<option>Falha ao buscar fontes</option>';failure(err.message);}}finally{if(!dead&&requestVersion===loadGeneration)q('[data-retry]').disabled=false;}
  }
  ...
  on(q('[data-retry]'),'click',()=>{void load(true);void refreshCommunity();});on(q('[data-overlay-retry]'),'click',()=>load(true));
