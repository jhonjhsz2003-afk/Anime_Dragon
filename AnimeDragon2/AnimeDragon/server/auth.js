...
const publicUser = u => ({id:u.id,name:u.username,email:u.email,bio:u.bio || '',avatar:avatar(u.avatar_url),visibility:u.visibility||'private',nameColor:u.name_color||'ice',avatarFrame:{x:u.avatar_x??50,y:u.avatar_y??50,zoom:u.avatar_zoom??100}});
...
export async function getUser(r,env) {
  ...
  return env.DB.prepare(`SELECT users.*,COALESCE(profile_privacy.visibility,'private') visibility,COALESCE(user_appearance.name_color,'ice') name_color,pm.x avatar_x,pm.y avatar_y,pm.zoom avatar_zoom FROM sessions JOIN users ON users.id=sessions.user_id LEFT JOIN profile_privacy ON profile_privacy.user_id=users.id LEFT JOIN user_appearance ON user_appearance.user_id=users.id LEFT JOIN profile_media pm ON pm.user_id=users.id WHERE sessions.id=? AND sessions.expires_at>?`).bind(await digest(token),new Date().toISOString()).first();
